...
1. An [OAuth client][kb-oauth-clients] ID and secret, [workload identity][kb-workload-identity-federation] client ID and audience, OR an [auth key][kb-auth-keys].
...
You may encounter this error when using an OAuth client. OAuth clients must have the [`auth_keys` scope](https://tailscale.com/kb/1215/oauth-clients#scopes) with one or more [tags][kb-tags], and the tags specified with `tags` must match all tags on the OAuth client.
